...
jQuery(document).ready( function($) {
  ...
  async function parseMIDI() {
    ...
    const tps = Math.round(midi.durationTicks / midi.duration);
    const tpr = tps / 4;
    const tpb = Math.round(midi.durationTicks / tpr);
    ...
    function trackDetails(track) {
      ...
      notes.forEach(note => {
        ...
        output[start] = {note: note.name, bars: bars, start: start, ticks: ticks};
        i++;
      });
    ...
    function midi2mod(midi) {
      ...
      for (var i=0; i<tpb; i++) {
          ...
          if (midi.hasOwnProperty(i)) {
              
            var note = midi[i].note,
                bars = midi[i].bars,
                pos  = midi[i].ticks,
                ni   = i + 1,
            ...
            if (first || pos > tick) { 
              ...
              tick = pos;
              output = output + '|' + alterNote(note) + '03......';
              c2 = '|...........';
              c3 = '|...........';
            }
            ...
            if (pos <= tick) {
              sim++;
              if (sim == 1) {
                output = output + '|' + alterNote(note) + '01......'; 
                c2 = ''; 
              }
              if (sim == 2) {
                output = output + '|' + alterNote(note) + '15......';
                c3 = '';   
              }
            }
